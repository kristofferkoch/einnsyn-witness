...
export async function postJson(url, { body, headers = {}, timeoutMs = 60000, retries = 3, family = 4 } = {}) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      const result = await once(url, { body, headers, timeoutMs, family });
      console.error(`[http] attempt ${attempt}/${retries} ok in ${Date.now() - started}ms`);
      return result;
    } catch (err) {
      lastErr = err;
      console.error(
        `[http] attempt ${attempt}/${retries} failed in ${Date.now() - started}ms: ${err.code || err.statusCode || ""} ${err.message}`,
      );
